          ...
          <div class="vx-row justify-center items-center w-full h-full">
            ...
            >
              <div style="display: flex; justify-content: center">
                <img src="@/assets/images/dashboard/Petcare.svg" />
              </div>
...
export default {
  ...
  methods: {
    ...
    async validateVc() {
      const vcJWT = localStorage.getItem('vcData');
      console.log(vcJWT)
      try {
        this.$vs.loading();
        ...
        const data1=JSON.parse(localStorage.getItem('orgId'));
        const data2=JSON.parse(localStorage.getItem('userd'));
        const did=localStorage.getItem('did');
        // console.log(data1,data2,did,'lllllllll');
        if(vc.vcDataModel.credentialSubject.id==did){
          this.mint();
        }else{
          this.$vs.notify({
                  title: 'Error',
        ...
        // console.log('Issued VC:', JSON.stringify(vc));
        // console.log(vc.vcDataModel.credentialSubject.id);
      } catch (error) {
        ...
        if (window.ethereum.networkVersion === this.network) {
          try {
            ...
            if (verifiedWallet) {
              ...
              localStorage.removeItem('orgDid');
