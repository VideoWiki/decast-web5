<template>
  <div v-if="dataLoaded">
    ...
    />
    <div v-else>
      <div class="w-full my-20 lg:block">
        <TheHeader />
      </div>
      <div class="flex video-container">
        ...
        <div v-else class="container md:py-10 py-2">
          <img
            src="https://s3-us-west-2.amazonaws.com/media.tmmdance.com/media/live-stream-not-started.png"
            alt="Event will start soon"
            class="w-full"
          />
        </div>
        <div class="md:py-10 pb-0 stream_detail">
          <h4 class="text-slate-500 text-xl font-medium">
            Started at : {{ start_time_new }}
          </h4>
          <h3 class="my-3 text-3xl font-semibold">
            Event name : {{ e_title }}
          </h3>
          <h4 class="text-slate-500 text-xl font-medium">
            Created by : {{ e_creator_name }}
          </h4>
          <p class="my-2 text-slate-500 text-2xl font-medium">
            {{ e_description }}
          </p>
...
import StreamingVerification from './StreamingVerification';
